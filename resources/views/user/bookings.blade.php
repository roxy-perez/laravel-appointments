        ...
        @else
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6">
                @foreach ($bookings as $booking)
                    <div
                        class="bg-white dark:bg-gray-700 overflow-hidden shadow-sm sm:rounded-lg {{ $booking->slot->slot_date->isPast() ? 'opacity-50' : '' }}">
                        <div class="p-6 text-gray-900 dark:text-cyan-100">
                            ...
                            <p class="text-sm text-gray-600 dark:text-white mb-2">{{ $booking->slot->business->phone }}
                            </p>
                            <p class="text-sm text-gray-600 dark:text-white">{{ $booking->slot->start_time }} -
